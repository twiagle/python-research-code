...
def main():
    ...
    num_of_image_per_video = 10
    type = ['train', 'val', 'test']
    # TIME = str(datetime.now())
    vggnet = Vgg19(vgg_model_path)#class
    vggnet.build()
    with tf.Session() as sess:
        ...
        for each in type:
            ...
            # feature extraction
            for idx, vf in enumerate(video_filename):#/home/tb/rnn_attention/Action_Recognition_using_Visual_Attention/data/image/a08/v01_s04_e04
                images_list = sorted(list(os.walk(vf))[0][-1], cmp=comp)
                print ('Processed' + str(idx + 1) + 'videos..')

                # # generate images_path
                cur_images_path = [vf + '/' + image for image in images_list]#/data/image/a08/v01_s04_e04/0.jpg 1.jpg...
                step = int(float(len(images_list)) / float(num_of_image_per_video))
                print(step)

                # Supplement
                if step == 0:#last image name * (17 - num of images)  less than 17 then add to 17 with last image
                ...
                all_feats[idx, :] = feats# idx ..to the end
