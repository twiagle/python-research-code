...
current_path = '/home/xf/abnormalDetection/data/'


def main():
    train_data = load_data(current_path + 'data_set/', 'train')
    ...
    solver = CaptioningSolver(model, data, n_epochs=200, batch_size=13, update_rule='adam',
                              learning_rate=0.0006, print_every=3, save_every=10,
                              pretrained_model=None, model_path=current_path + 'model/lstm/',
                              test_model=current_path + 'model/lstm/model-100', log_path=current_path + 'log/',
