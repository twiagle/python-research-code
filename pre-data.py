...
def seprate_data(folders, read_path, save_path):
    ...
    for attr in attributes:
        for s in sets:
            exec (attr + '_' + s + '=[]')#creat list

    # process in each folder
    for each in folders:
        path = read_path + each + '/'

        video_names = load_pickle(path + each + '_video_names.pkl')#list v01_s01_e00
        label = load_pickle(path + each + '_label.pkl')#str
        number = len(video_names)#number of folder in a01, same kind label

        Video_Filenames = [path + per + '/' for per in video_names]  # video_filenames : /data/image/a01/v01_s01_e00
        Video_Names = [each + '_' + name for name in video_names]  # VideoIds a01_v01_s01_e00
        Labels = [label] * number  # labels for each Video_Filenames, [str]*3 append copy 3 times

        # inside_mix up -- different people's mixing up
        Labels, Video_Filenames, Video_Names = mix_up(Labels, Video_Filenames, Video_Names)

        # list_lization
        Labels = list(Labels)#np.array --> list
...
#create v01_s01_e00 ... folders
def video2image(rpath, spath, n):
    # '-r ' + str(n) +
    strcmd = 'ffmpeg -i ' + '"' + rpath + '"' + ' -r 12 ' + ' -vframes ' + str(
...
def main():
    ...
    video_path = '/home/xf/abnormalDetection/ped1avi/'
    data_path = '/home/xf/abnormalDetection/data/'
    folders = ['abnormal', 'normal']#no 7,10

    # folder-type to label
    video_labels_dic = {'abnormal': 'abnormal',
                        'normal': 'normal'}
    for type in folders:
        cur_video_path = video_path + type + '/'#/home/tb/rnn_attention/Action_Recognition_using_Visual_Attention/zzp/multiview_action_videos/a01/
        cur_image_path = data_path + 'image/' + type + '/'#/home/tb/rnn_attention/Action_Recognition_using_Visual_Attention/data/image/a01/

        # obtain the label of videos
        try:
            label = open(cur_video_path + 'label-' + type + '.txt').readline()[:-1]#no such txt, goto except
        except:
        ...
        save_pickle(label, cur_image_path + type + '_label.pkl')#/home/tb/rnn_attention/Action_Recognition_using_Visual_Attention/data/image/a01
        ...
        for index, name in enumerate(video_txt):
            print ('video' + name[:-1] + 'process ... ')# name[:-1] str begin to end 

            # remove the data type
            name = name[:-5]
            rpath = cur_video_path + name + '.mp4'#/home/tb/rnn_attention/Action_Recognition_using_Visual_Attention/zzp/multiview_action_videos/a01
            spath = cur_image_path + name + '/'#/home/tb/rnn_attention/Action_Recognition_using_Visual_Attention/data/image/a01/v01_s01_e00
    ...
    label_to_idx = {'abnormal': 1, 'normal': 0}
